...
void worklist::set_param(float ave_wt, float ave_degree) {
	...
	quantum = ave_wt / (1 << QUAN_SHIFT);

	float f = (((float) ave_wt / (float) ave_degree) / INIT_FACTOR) / quantum;
	unsigned pow2 = 1;
	unsigned u = 0;
	do {
	...
	printf("MAX_CON_BAG %u, MINI_GRAIN %u, ave_wt %f, ave_degree %f, quantum %f, delta init %u (%f)\n", MAX_CON_BAG, mini_grain, ave_wt, ave_degree, quantum, delta_init, f);
...
#define PROFILE_WORK 32
#define CHANGE_BAG_WAIT 1
#define CHANGE_DELTA_WAIT 1
#define PROBE_LOW_WORK_MIN 4
#define CONSEC_LOW_BAG_TSH 3
#define AGG_LOW_BAG_LEAVE 3
#define CONSEC_LOW_PROB_TSH (CONSEC_LOW_BAG_TSH*AGG_LOW_BAG_LEAVE + 5)

#define HIGH_CLIP 0.6f
#define HIGH_CLIP_RANGE 6
#define LOW_DELTA_BAN 3

#define INIT_SKIP_CONBAG 3
#define MIN_CON_BAG 2

#define INIT_MUL 0.2f
...
#define CURVE_HI_P_COEFF 0.3f
#define CURVE_HI_UTL_COEFF  1.75f
#define CURVE_HI_P_POW 0.6f
#define CURVE_HI_UTL_POW 0.6f

#define CURVE_LOW_P_COEFF 0.1f
#define CURVE_LOW_UTL_COEFF 0.5f
#define CURVE_LOW_P_POW 0.15f
#define CURVE_LOW_UTL_POW 0.15f
...
#define SMALL_DELTA_INC_TSH 3
#define SMALL_WAIT_BAG_FROM_PROBE 4
#define SMALL_WAIT_WORK_FROM_PROBE 4.0f

#define CLEAR_LOW_INTERVAL 0.1f
#define SAME_BAG_DEC_INTERVAL 0.4f
		...
		if (last_assign_ptr[updated_bag % NUM_BAG] < read_ptr[updated_bag % NUM_BAG]) {
									...
									if ((cur_P < hi_P) && (util < hi_UTL)) {
										...
										if (util < transit_util) {
											...
											if (forgive_count > DEC_DELTA_FORGIVE) {
												...
												remaining_work = __shfl_sync(FULL_MASK, remaining_work, 0);
			...
			if ((updated_bag != current_bag) || delta_change) {
				...
				if (updated_bag != current_bag) {
					...
					cur_dist = set_bits(cur_dist, 0, 0, next_delta);
					if (get_lane_id() == 0) {
...
__device__ __forceinline__ unsigned worklist::dist_to_bag_id_int(unsigned bag_id, node_data_type dst_dist) {
	...
	unsigned delta = bfg_get_delta(delta_info);
...
//return (cur_bag + 1) % NUM_BAG;
		...
		return (cur_bag + bag_offset) % NUM_BAG;
	}
}
