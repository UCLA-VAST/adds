...
unsigned CSRGraph::allocOnHost() {
	...
	size_t mem_usage = ((nnodes + 1) + nedges) * sizeof(index_type) + (nedges) * sizeof(edge_data_type) + (nnodes) * sizeof(node_data_type);
...
struct EdgeIterator {
	...
	__device__ index_type dst() const {
		return g->edge_dst[s];
	}

	__device__ edge_data_type data() const {
